...
// ValidateStruct validates a struct and returns a map of field-specific error messages
func ValidateStruct(s interface{}) map[string]string {
	...
	for _, fieldErr := range validationErrors {
		// Construct message key like `employees.name.required`
		field := fieldErr.StructNamespace()
		tag := fieldErr.Tag()
		key := fmt.Sprintf("%s.%s", field, tag)

		// Use specific message if available, fallback to generic tag-based message
		message := validationMessages[key]
		if message == "" {
			message = validationMessages[tag]
		}

		// Format the message if it has parameters
		if fieldErr.Param() != "" {
			message = fmt.Sprintf(message, fieldErr.Param())
		}

		// Store the error using the field name
